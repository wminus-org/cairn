...
- **200m ≥ d > r — resolving.** The synthesised waveform preview draws but blurred; the photo placeholder renders coarse. Both sharpen continuously with `p`. Distance number stays on screen.
- **d ≤ r — open.** Full-resolution image, sharp waveform, autoplay once, accent flips to amber `#D9A441`.

**The waveform preview is synthesised, not fetched.** Hash the stone `id` into 24 bucket values 1–6 (per `reference/design-system.md` § As a preview on a stone card) with a pure function — same id, same stack, every render, on every device. No server field feeds it, and there is none to feed it: CRN-005's `approaching` band ships no `peaks`, `duration_ms` or `thumb`, and those columns do not exist. Do not reach for `Math.random()` at 14:50 — it reshuffles the stack on every re-render and flickers.
...
- **Photo (Approach band):** the server ships no image bytes before `radius_m`. Render a synthetic placeholder — the stone's `image_aspect_ratio` box filled with base + a deterministic noise/blocking pattern seeded from the stone `id` — under the same `BlurView`. The real image appears in one step when `band` is `unlocked`. There is nothing to pixelate and no thumbnail derivative to fetch; blur is a rendering treatment, never a fetch decision.
...
- [ ] At 150m, the waveform is visibly blurred and the photo placeholder is visibly coarse, and the network response that populated that card contains no `image_url`, no `audio_url` and no `transcript` key. Check it in the inspector, not by reading the code.
- [ ] The same stone renders an identical stack shape across a re-render, an app restart, and a second device.
...
- **There is no nearest-neighbour flag** on React Native `Image` or `expo-image`. Do not go looking for one. You get crunchy pixels by drawing them yourself in the placeholder, not by asking the image pipeline for them.
- **Do not invent a degraded derivative to interpolate towards.** There is no `thumb_url`, no `peaks` and no `duration_ms` column, and adding one at 14:45 is not the fix. Hold the single source image the server released and let the `BlurView` intensity curve carry the whole interpolation.
