    ...
    <View style={styles.row}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.body}>
          {/* Animated.View, not a plain View: `degrade.opacity` is a native
              driver node, so the distance ease never re-renders this row. */}
...
const styles = StyleSheet.create({
  ...
  // never a third size.
  transcript: { ...type.body, color: colors.t60, marginTop: s.unit, maxWidth: '100%' },
  // The line that stands in for a transcript still in flight. Same size as the
  // transcript — the "never a third size" rule holds — but fainter, and not
  // amber: amber is the proximity signal on this screen and nothing else may
  // borrow it.
  transcriptPending: { ...type.body, color: colors.t20, marginTop: s.unit },
